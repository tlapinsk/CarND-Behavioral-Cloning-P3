...
- After receiving feedback, I got a great tip! I kinda wish I had caught it prior to submitting, but oh well. In model.py, I read the images using cv2, which brings in images in BGR format. In drive.py, I never converted from rgb to bgr, so it wasn't processing in the same color space. I ended up making the small change as suggested and my car improved even more! See drive.py line 64 for the change. No more wild swings and it never touches the edge now. Huge shoutout to the reviewer for bringing this up, and I'll definitely be more careful about how I read in images (e.g. what color space) going forward.
